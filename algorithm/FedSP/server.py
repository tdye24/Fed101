...
class Server(BASE):
    def __init__(self,
                 ...
                 decay_step=200,
                 note=''):
        BASE.__init__(self, algorithm='fedsp', seed=seed, epoch=epoch, model_name=model_name, dataset_name=dataset_name, lr=lr, batch_size=batch_size,
        ...
        self.test_writer = SummaryWriter(
            f'/home/tdye/Fed101/visualization/fedsp/{dataset_name}_{model_name}_C{clients_per_round}_E{epoch}_B{batch_size}_lr{lr}_val_{note}')
    ...
    def print_optim(self):
        for c in self.clients:
            c.set_shared_params(self.params)
    ...
    def save_model(self):
        path = f'/home/tdye/Fed101/result/fedsp/{self.dataset_name}_{self.model_name}_C{self.clients_per_round}_E{self.epoch}_B{self.batch_size}_lr{self.lr}_{self.note}'
    ...
    def save_clients_models(self):
        for client in self.clients:
            path = f'/home/tdye/Fed101/result/fedsp/{self.dataset_name}'
            if not os.path.exists(path):
                os.makedirs(path)
            path = f'{path}/{client.user_id}.pkl'
            print(f"client {client.user_id}'s model saved to：{path}")
            torch.save(client.model, path)
